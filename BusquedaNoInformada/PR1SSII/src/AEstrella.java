...
public class AEstrella {
	...
	AEstrella(State si, int semilla){
	...
	public ArrayList<Nodo> sucesores(Nodo nodo){
		...
		try {
			for(int i=0; i<acciones.size(); i++) {
				...
				nuevoNodo.setHeuristica(funcionHeuristica(nuevoNodo));
	...
	public int funcionHeuristica(Nodo nodo) {
		...
		int x = nodo.estado.m_boardSize- nodo.estado.m_agentPos.row;
		int y=0;
	...
	public void busqueda() {
		...
		elegido.setCoste(1+Math.max(Math.abs(elegido.estado.m_agentPos.row-elegido.estado.m_boardSize), Math.abs(elegido.estado.m_agentPos.col-elegido.estado.m_boardSize)));
	...
	public static void main(String[] args) {
		...
		AEstrella srs = new AEstrella(state,seed2);
		srs.agente.color=0;
		srs.busqueda();
