...
public class PrimeroMejor {
	...
	public static final Comparator<Nodo> porHeuristica = new Nodo.evaluadorHeuristica();
	long nodosGenerados=0;
	long tiempoBusqueda;
	
	PrimeroMejor(State si, int semilla){
	...
	public ArrayList<Nodo> sucesores(Nodo nodo){
		...
		try {
			for(int i=0; i<acciones.size(); i++) {
				...
				nuevoNodo.setHeuristica(nuevoNodo.estado.m_boardSize- nuevoNodo.estado.m_agentPos.row); //la heuristica va a ser la distancia entre la posicion y el final del tablero
				nuevoNodo.setProfundidad(nodo.getProfundidad()+1);
	...
	public void busqueda() {
		...
		PriorityQueue<Nodo> frontera = new PriorityQueue<Nodo>(porHeuristica);
		Boolean encontrada = false;
		State estado_actual =  this.estado_inicial;

		Nodo elegido = new Nodo(estado_actual); //Creamos un nodo con el estado inicial
		elegido.setHeuristica(elegido.estado.m_boardSize- elegido.estado.m_agentPos.row); //la heuristica va a ser la distancia entre la posicion y el final del tablero
		elegido.setProfundidad(1);
		 ...
		 */
			...
			while(encontrada == false) {
				if(estado_actual.isFinal()) {
					...
					frontera.addAll(sucesores(elegido));
					Action comprobar = frontera.element().getAction();
	...
	public static void main(String[] args) {
		...
		PrimeroMejor srs = new PrimeroMejor(state,seed2);
		srs.agente.color=0;
		srs.busqueda();
