...
public class MsgConexion {
    ...
    public QueueConnectionFactory jmsFactoria = null; //Factoria de conexiones
    ...
    public boolean inicializaCola() throws JMSException, NamingException{
        if(contexto==null){
            ...
            jmsFactoria = (QueueConnectionFactory) contexto.lookup("jms/QueueConnectionFactory");
            //obtiene la cola (ha debido ser creada externamente)
            cola = (Queue) contexto.lookup(nombreCola);
            //Ahora crea la conexion a la cola
            conexionCola=jmsFactoria.createQueueConnection();
            //Crea la sesion
